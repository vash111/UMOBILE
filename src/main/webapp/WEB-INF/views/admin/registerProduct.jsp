...
<body class="sb-nav-fixed">
								...
								<form action="product/register" method="post"
									...
									<div class="form-group">
										<label for="price">가격</label> <input type="text" id="price"
											name="price" required placeholder="1,000,000 형태로 입력하세요." oninput="validatePrice()">
									</div>
	...
	<script src="/resources/js/datatables-simple-demo.js"></script>
        ...
        function validatePrice() {
            ...
            var regex = /^[0-9,]*$/;  // 숫자와 쉼표만 허용

            if (!regex.test(price)) {
                alert("가격은 숫자와 쉼표(,)만 입력할 수 있습니다.");
        ...
        function handleSubmit(event) {
            // 가격 유효성 검사
            if (!validatePrice()) {
                // 가격이 유효하지 않으면 폼 제출을 막음
                event.preventDefault();
            } else {
                // 유효하면 폼 제출 허용 (자동으로 제출됨)
                return true;
            }
        }
    </script>
